...
paths.properties = function (settings, routes) {
    ...
    routes.forEach((route) => {
        ...
        if (payloadType.toLowerCase() === 'json') {
            ...
            // If the route is using a custom validation function, accepts anything
            if (typeof route.payloadParams === 'function') {
                route.payloadParams = Joi.any();
            }
        ...
        let headerParams = Properties.toParameters(internals.getJOIObj(route, 'headerParams'), swagger.definitions, 'header');
